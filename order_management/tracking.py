...
class PositionManager:
    ...
    async def calculate_position_from_trades(self, exchange: str, symbol: str, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> Position:
        """
        Calculate position for exchange/symbol from database trades within a time window.
        ...
        try:
            # Get trades for this exchange/symbol from database within the time window
            trades = await self.trade_repository.get_trades_by_exchange_symbol(
    ...
    async def get_all_positions(self, symbol: Optional[str] = None, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> List[Position]:
        """
        Get all positions, optionally filtered by symbol and time window.
        ...
        try:
            # Get all unique exchange/symbol combinations from database within the time window
            ...
            for exchange, db_symbol in exchange_symbols:
                ...
                position = await self.calculate_position_from_trades(
                    exchange, db_symbol, start_time=start_time, end_time=end_time
                )
    ...
    async def get_positions_by_exchange_async(self, exchange: str, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> List[Position]:
        """
        Get all positions for a specific exchange from database within a time window.
        ...
        try:
            # Get all symbols for this exchange from database within the time window
            symbols = await self.trade_repository.get_symbols_by_exchange(
                exchange, start_time=start_time, end_time=end_time
            )
    ...
    async def get_net_position(self, symbol: str, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> Dict[str, Any]:
        """
        Get net position across all exchanges for a symbol within a time window.
        ...
        Returns:
            Net position data
        """
        positions = await self.get_all_positions(symbol, start_time=start_time, end_time=end_time)
    ...
    async def get_all_net_positions(self, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> Dict[str, Dict[str, Any]]:
        """
        Get net positions for all symbols across all exchanges within a time window.
        Calculates from database trades.
        ...
        try:
            # Get all unique symbols from database within the time window
            symbols = await self.trade_repository.get_unique_symbols(start_time=start_time, end_time=end_time)
            
            for symbol in symbols:
                net_position = await self.get_net_position(symbol, start_time=start_time, end_time=end_time)
    ...
    async def summarize_positions(self, start_time: Optional[datetime] = None, end_time: Optional[datetime] = None) -> Dict[str, Any]:
        """
        Create position summary from database trades within a time window.
        ...
        """
        positions = await self.get_all_positions(start_time=start_time, end_time=end_time)
