...
class PassiveLineConfig:
    ...
    quantity_coefficient_method: str = "volume"  # 'inventory', 'volume', 'both'
...
class StackedMarketMakingStrategy(BaseStrategy):
    ...
    def _get_required_symbols(self) -> List[str]:
        ...
        # CRITICAL: Always add BERA/USDT for conversion purposes (even if USDT not in config)
        ...
        # Ensure FDUSD/USDT is always included for Binance reference pricing
        symbols.add('FDUSD/USDT')
        self.logger.info("🔧 CONVERSION SUPPORT: Ensured FDUSD/USDT is included for reference pricing")

        # Cross-conversion symbols (NON-USDT/USDT) - only for available currencies
        for quote_currency in available_quote_currencies:
            if quote_currency != 'USDT' and quote_currency in known_reference_pairs:
    ...
    def _build_exchange_symbol_map(self, primary_symbols: List[str]) -> Dict[str, List[str]]:
        ...
        # Support matrix with cross-currency conversion approach:
        # Based on actual exchange capabilities from logs
        support_matrix = {
            'binance_spot': ['BERA/USDT', 'BERA/USDC', 'BERA/BTC', 'BERA/BNB', 'BERA/TRY', 'BERA/FDUSD'],  # Binance DOES have BERA/USDC
            'binance_perp': ['BERA/USDT'],  # ONLY USDT on perp
            'bybit_spot': ['BERA/USDT', 'BERA/USDC'],  # Both pairs available
            'bybit_perp': ['BERA/USDT'],    # ONLY USDT on perp
            'mexc_spot': ['BERA/USDT'],  # Only USDT (ignoring BERA/USDC per user request)
            'bitget_spot': ['BERA/USDT'],   # USDT for conversion and trading
            'gateio_spot': ['BERA/USDT'],   # USDT for conversion and trading
            'hyperliquid_perp': ['BERA/USDC']  # ONLY USDC on hyperliquid perp
        }

        for exchange in self.exchanges:
            supported_symbols = support_matrix.get(exchange, [])
            # Filter to only symbols we actually want to trade
            available_symbols = [s for s in primary_symbols if s in supported_symbols]
    ...
    async def _strategy_loop(self) -> None:
        ...
        all_symbols = getattr(self, '_cached_required_symbols', self._get_required_symbols())
        ...
        while self.running:
            try:
                ...
                # Process each exchange with its supported symbols (place missing orders after cancellations)
                symbol_tasks = []
                for exchange in self.exchanges:
                    ...
                    for symbol in supported_symbols:
                        ...
                        for line in self.active_tob_lines.values():
                            symbol_tasks.append(self._process_tob_line_for_exchange_symbol(line, exchange, symbol))

                        # Process Passive lines for this symbol on this exchange
                        for line in self.active_passive_lines.values():
                            symbol_tasks.append(self._process_passive_line_for_exchange_symbol(line, exchange, symbol))
                ...
                # Execute all line processing concurrently
                if symbol_tasks:
    ...
    async def _place_missing_passive_orders_for_exchange_symbol(self, line: ActivePassiveLine, exchange: str, symbol: str):
        ...
        try:
            ...
            adjusted_spread = self._apply_inventory_spread_adjustment(base_spread)

            # Apply coefficient adjustments
            final_spread = adjusted_spread * line.current_spread_coefficient

            # Ensure spread bounds
            final_spread = max(config.min_spread_bps, min(config.max_spread_bps, final_spread))

            # Calculate bid and ask prices
            spread_decimal = final_spread / Decimal('10000')
            passive_bid_price = aggregated_price * (Decimal('1') - spread_decimal)
            passive_ask_price = aggregated_price * (Decimal('1') + spread_decimal)
            ...
            self.logger.info(f"🔧 MARKET DATA DEBUG: {exchange} {resolved_symbol} -> bid={local_best_bid}, ask={local_best_ask}")
        ...
        try:
            ...
            if config.spread_from_inventory and inventory_price:
                ...
                if target_inventory and target_inventory != 0:
                    try:
                        inv_coeff = (current_inventory - target_inventory) / target_inventory
                    except Exception:
                        inv_coeff = Decimal('0')
                    # Clamp
        ...
        try:
            ...
            if needs_conversion:
                ...
                if converted_price is not None:
                    ...
                    self.logger.warning(f"❌ Conversion pricing failed for {symbol}, using direct multi-exchange pricing")
    ...
    def _exchange_has_native_pair(self, exchange: str, symbol: str) -> bool:
        ...
        # Define which pairs actually exist natively on each exchange
        # Based on API errors and actual capabilities
        native_pairs = {
            'binance_spot': ['BERA/USDT', 'BERA/USDC', 'BERA/BTC', 'BERA/BNB', 'BERA/TRY', 'BERA/FDUSD'],  # Binance DOES have BERA/USDC (confirmed from logs)
            'binance_perp': ['BERA/USDT'],
            'bybit_spot': ['BERA/USDT', 'BERA/USDC'],  # Both USDT and USDC native on Bybit
            'bybit_perp': ['BERA/USDT'],
            'mexc_spot': ['BERA/USDT'],  # Only USDT native (BERA/USDC API error)
            'bitget_spot': ['BERA/USDT'],
            'gateio_spot': ['BERA/USDT'],
            'hyperliquid_spot': [],  # No native pairs on hyperliquid spot
            'hyperliquid_perp': ['BERA/USDC']  # ONLY USDC on hyperliquid perp
        }

        return symbol in native_pairs.get(exchange, [])
    ...
    async def _check_tob_timeouts_for_symbol(self, line: ActiveTOBLine, symbol: str):
        """Check for TOB line order timeouts for a specific symbol."""
            ...
            if drift_bps > line.config.drift_bps:
                ...
                for side in ['bid', 'ask']:
                    if side in line.active_orders.get(symbol, {}).get(exchange, {}):
                        await self._cancel_tob_order_for_symbol(line, exchange, symbol, side, 'drift')
        ...
        try:
            ...
            for exchange in list(line.active_orders[symbol].keys()):
                ...
                if last_bid_price is not None and abs(last_bid_price - intended_bid) > Decimal('0'):
                    ...
                    await self._cancel_tob_order_for_symbol(line, exchange, symbol, 'bid', 'replace')
                ...
                if last_ask_price is not None and abs(last_ask_price - intended_ask) > Decimal('0'):
                    ...
                    await self._cancel_tob_order_for_symbol(line, exchange, symbol, 'ask', 'replace')
        except Exception as e:
    ...
    async def _check_passive_timeouts_for_symbol(self, line: ActivePassiveLine, symbol: str):
        """Check for passive line order timeouts for a specific symbol."""
        now = datetime.now(timezone.utc)
        timeout_delta = timedelta(seconds=line.config.timeout_seconds)
        ...
        try:
            ...
            for exchange in list(line.active_orders.get(symbol, {}).keys()):
                ...
                if drift_bps > line.config.drift_bps:
                    ...
                    # Cancel only this exchange's passive orders for the symbol
                    for side in ['bid', 'ask']:
                        if side in line.active_orders.get(symbol, {}).get(exchange, {}):
                            await self._cancel_passive_order_for_symbol(line, exchange, symbol, side, 'drift')
    ...
    async def _check_all_order_timeouts_and_drift(self):
        """Check all active orders for timeouts and drift (like passive quoting strategy)."""
        try:
    ...
    def _log_performance_summary(self, last_cycle_time_ms: float):
        ...
        inv_coeff = 0.0
        if target_inv > 0:
            coefficient = excess_inv / target_inv
    ...
    def get_strategy_state(self) -> Dict[str, Any]:
        ...
        if target_inventory > 0:
            coefficient = excess_inventory / target_inventory
