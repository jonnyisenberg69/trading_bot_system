...
class VolumeWeightedTopOfBookStrategy(BaseStrategy):
    ...
    async def _stop_strategy(self) -> None:
        ...
        # Cancel tasks
        tasks_to_cancel = [self.main_task, self.ma_update_task, self.inventory_update_task]
    ...
    def _should_track_time_for_side(self, side: str, current_price: Decimal) -> bool:
        ...
        if not self.inventory_price:
            ...
            self.logger.debug(f"No inventory price available for {side} time tracking")
            return False
    ...
    async def _calculate_order_size(self, level: PriceLevelEntry, exchange: str) -> Decimal:
        ...
        if level.side == "bid":  # buying
            if self.current_inventory + remaining_to_execute > self.target_inventory:
                # This would push us beyond our target, limit it
                max_allowed = max(Decimal('0'), self.target_inventory - self.current_inventory)
                if max_allowed < remaining_to_execute:
                    structlog.get_logger().info(
                        "Limiting buy size to avoid exceeding target inventory",
                        side=level.side,
                        original_size=float(remaining_to_execute),
                        limited_size=float(max_allowed),
                        current_inventory=float(self.current_inventory),
                        target_inventory=float(self.target_inventory)
                    )
                    remaining_to_execute = max_allowed
        else:  # selling
            if self.current_inventory - remaining_to_execute < self.target_inventory:
                # This would push us below our target, limit it
                max_allowed = max(Decimal('0'), self.current_inventory - self.target_inventory)
                if max_allowed < remaining_to_execute:
                    structlog.get_logger().info(
    ...
    async def _place_level_order(
        ...
        try:
            ...
            # For buy orders: check if current_inventory + size > target_inventory
            # For sell orders: check if current_inventory - size < target_inventory
            would_exceed_target = False
            if order_side == 'buy' and self.current_inventory + size > self.target_inventory:
                would_exceed_target = True
                excess_amount = self.current_inventory + size - self.target_inventory
                # Adjust size to exactly hit target
                adjusted_size = size - excess_amount
                if adjusted_size <= Decimal('0.00001'):  # Too small to execute
                    structlog.get_logger().info(
                        "Skipping order that would exceed target inventory",
                        exchange=exchange,
                        side=order_side,
                    ...
                    structlog.get_logger().info(
                        "Adjusting order size to avoid exceeding target inventory",
                        exchange=exchange,
                        side=order_side,
                        original_size=float(size),
                    ...
                    size = adjusted_size
            elif order_side == 'sell' and self.current_inventory - size < self.target_inventory:
                would_exceed_target = True
                excess_amount = self.target_inventory - (self.current_inventory - size)
                # Adjust size to exactly hit target
                adjusted_size = size - excess_amount
                if adjusted_size <= Decimal('0.00001'):  # Too small to execute
                    structlog.get_logger().info(
                        "Skipping order that would fall below target inventory",
                        exchange=exchange,
                        side=order_side,
                        original_size=float(size),
                        current_inventory=float(self.current_inventory),
                    ...
                    structlog.get_logger().info(
                        "Adjusting order size to avoid falling below target inventory",
                        exchange=exchange,
                        side=order_side,
                        original_size=float(size),
                        adjusted_size=float(adjusted_size),
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            'strategy': 'volume_weighted_top_of_book',
    ...
    async def _publish_performance_data(self) -> None:
        ...
        try:
            ...
            full_data = {
                ...
                "strategy": "volume_weighted_top_of_book",  # Use the strategy key for frontend
                "symbol": self.symbol,
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        enhanced_dict = {
            'strategy_type': 'volume_weighted_top_of_book',
    ...
    async def _exchange_monitor_loop(self, exchange: str) -> None:
        ...
        while self.running:
            try:
                ...
                #     self.logger.warning(f"Stale orderbook data for {exchange}, skipping order checks")
                #     await asyncio.sleep(0.05)
                #     continue
                ...
                for level in self.bid_levels + self.offer_levels:
                    if exchange in level.active_orders:
                        # Check if order is still at the top of the book
                        ...
                        # Check drift with fresh market data
                        if exchange in level.order_placed_price:
                            placed_price = level.order_placed_price[exchange]
    ...
    async def _handle_risk_ws_message(self, payload: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # If we still have no size, nothing to do
