#!/usr/bin/env python
"""
Exchange-by-Exchange Pipeline Test
...
Each exchange is tested thoroughly with detailed logging and validation.
"""
...
class ExchangePipelineTester:
    """Test complete pipeline for individual exchanges."""
    ...
    def __init__(self):
        ...
        # Trading configuration for minimal viable tests
        self.test_configs = {
            'binance_spot': {
                'symbol': 'BTC/USDT',
                'amount': 0.0001,  # ~$10 order
                'order_type': 'market',
                'endpoints': {
            ...
            'hyperliquid_perp': {
                'symbol': 'BTC',  # Hyperliquid uses single symbol format
                'amount': 0.001,
                'order_type': 'market',
                'endpoints': {
    ...
    async def setup(self):
        ...
        logger.info("🔧 Setting up Exchange Pipeline Tester...")
    ...
    async def _get_or_create_exchange_id(self, exchange_name: str) -> Optional[int]:
        ...
        try:
            ...
            new_exchange = ExchangeModel(
                name=exchange_name,
                type=self.test_configs.get(exchange_name, {}).get('market_type', 'spot'),
                is_active=True
            )
    ...
    async def test_all_exchanges(self):
        ...
        logger.info("\n" + "="*80)
        logger.info("🚀 STARTING EXCHANGE-BY-EXCHANGE PIPELINE TESTS")
        logger.info("="*80)
        ...
        for exchange_name in self.test_configs.keys():
            ...
            # Wait between tests
            logger.info(f"⏳ Waiting 5 seconds before next exchange...")
    ...
    async def test_single_exchange(self, exchange_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if messages_received:
                ...
                logger.info(f"📨 {exchange_name} - Messages received! Orders: {len(self.received_messages['order_updates'])}, Trades: {len(self.received_messages['trade_notifications'])}")
            else:
    ...
    async def _print_final_summary(self):
        ...
        if fully_successful >= 6:
            ...
            logger.info(f"\n❌ NEEDS WORK: Only {fully_successful}/8 exchanges working")
...
async def main():
    """Main test execution."""
    tester = ExchangePipelineTester()
    
    try:
        await tester.setup()
        await tester.test_all_exchanges()
        
        logger.info("\n✅ EXCHANGE-BY-EXCHANGE PIPELINE TEST COMPLETED!")
