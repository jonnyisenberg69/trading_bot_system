#!/usr/bin/env python
"""
Ultra Optimized Exchange-by-Exchange Pipeline Test

Final version that addresses all remaining issues:
- Binance: Increased to 0.05 SOL (~$12.5) to ensure notional requirements
- Gate.io/Bitget: Fixed ticker fetching using exchange.fetch_ticker() directly
- Hyperliquid: Fixed order sizing with proper decimal handling
- All exchanges: Enhanced error handling and debugging
"""
...
class UltraOptimizedTester:
    """Ultra optimized test with all remaining issues fixed."""
    ...
    def __init__(self):
        ...
        # ULTRA OPTIMIZED configuration with all fixes
        self.test_configs = {
            'binance_spot': {
                'symbol': 'SOL/USDT',
                'amount': 0.05,  # Increased to ~$12.5 (well above $5 minimum)
                'order_type': 'market',
                'endpoints': {
            ...
            'bybit_spot': {
                'symbol': 'SOL/USDT',
                'amount': 0.03,  # Already working perfectly
                'order_type': 'market',
                'endpoints': {
            ...
            'bybit_perp': {
                'symbol': 'SOL/USDT:USDT',
                'amount': 0.15,  # Already working perfectly
                'order_type': 'market',
                'endpoints': {
                    'private': 'wss://stream.bybit.com/v5/private'
                }
            ...
            'hyperliquid_perp': {
                'symbol': '@1',  # ETH perpetual
                'amount': 0.01,  # Increased to 0.01 (~$40)
                'order_type': 'market',
                'endpoints': {
    ...
    async def setup(self):
        ...
        logger.info("🔧 Setting up Ultra Optimized Tester...")
    ...
    async def test_all_exchanges(self):
        ...
        logger.info("\n" + "="*80)
        logger.info("🚀 ULTRA OPTIMIZED EXCHANGE PIPELINE TEST")
        logger.info("="*80)
    ...
    async def test_single_exchange(self, exchange_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 4: Place a test order with ULTRA OPTIMIZED parameters
            logger.info(f"💼 {exchange_name} - Placing ultra optimized test order...")
            test_config = self.test_configs[exchange_name]
            
            order = None
            
            # Handle different order types with ULTRA FIXES
            if exchange_name in ['gateio_spot', 'bitget_spot']:
                # Use FIXED ticker fetching method
                order = await self._place_market_buy_with_cost_ultra_fixed(connector, test_config)
            elif exchange_name == 'hyperliquid_perp':
                # Hyperliquid with ULTRA fixed sizing
                order = await self._place_hyperliquid_order_ultra_fixed(connector, test_config)
            else:
                # Standard market order with ultra optimized size
    ...
    async def _place_market_buy_with_cost_ultra_fixed(self, connector, test_config: Dict[str, Any]):
        """Place market buy order with ULTRA FIXED ticker method."""
        try:
            # Use the underlying exchange object directly for ticker
            logger.info(f"Fetching ticker for {test_config['symbol']} using exchange.fetch_ticker()")
            ticker = await connector.exchange.fetch_ticker(test_config['symbol'])
            
            if not ticker or not ticker.get('last'):
                raise Exception("Could not get current price from exchange.fetch_ticker()")
            
            current_price = float(ticker['last'])
            cost = test_config.get('cost', 12.5)  # $12.5 worth
            ...
            logger.error(f"Error placing market buy with cost (ultra fixed): {e}")
            raise e
    
    async def _place_hyperliquid_order_ultra_fixed(self, connector, test_config: Dict[str, Any]):
        """Place order on Hyperliquid with ULTRA FIXED sizing."""
        try:
            symbol = test_config['symbol']  # '@1' for ETH perp
            amount = test_config['amount']  # Increased to 0.01
            
            logger.info(f"Placing Hyperliquid order: {amount} of {symbol} (ultra fixed)")
            
            # Use precise decimal formatting for Hyperliquid
            precise_amount = Decimal(str(amount)).quantize(Decimal('0.001'))
            ...
            logger.error(f"Error placing Hyperliquid order (ultra fixed): {e}")
            raise e
    ...
    async def _print_final_summary(self):
        ...
        logger.info("\n" + "="*80)
        logger.info("📊 ULTRA OPTIMIZED TEST RESULTS")
        logger.info("="*80)
...
async def main():
    ...
    try:
        ...
        logger.info("\n✅ ULTRA OPTIMIZED TEST COMPLETED!")
