...
from utils.symbol_utils import get_normalized_symbol_for_aggregation, convert_symbol_for_exchange
    ...
    try:
        ...
        return {
            "system": {
                ...
                "uptime": datetime.now(timezone.utc).isoformat(),  # Placeholder
                "version": "1.0.0"
            },
            "bots": {
                "total": len(all_bots),
...
async def get_dashboard_overview(
    ...
    try:
        ...
        # Convert running bots to dict with live performance data
        running_bots_data = []
        for bot in running_bots:
            bot_dict = await bot.to_dict(bot_manager.redis_client)
            running_bots_data.append(bot_dict)
        ...
        return {
            ...
            "running_bots": running_bots_data,
...
async def get_accounts(
    ...
    try:
        ...
        # Group bots by strategy/account to create logical accounts
        accounts_dict = {}
        
        # Process each bot instance to group by account
        for bot in bot_instances:
            # Determine account name from bot config or strategy
            account_name = "Main Account"  # Default account name
            
            # Check if bot has a specific account configured
            if hasattr(bot, 'config') and isinstance(bot.config, dict):
                account_name = bot.config.get('account_name', account_name)
            
            # If not in dict, create new account entry
            ...
            # Add bot info to account
            accounts_dict[account_name]["bot_count"] += 1
            if bot.status == "running":
                accounts_dict[account_name]["running_bots"] += 1
            
            # Add symbol
            if bot.symbol:
                accounts_dict[account_name]["symbols"].add(bot.symbol)
            
            # Add strategy
            if bot.strategy:
                accounts_dict[account_name]["strategies"].add(bot.strategy)
            
            # Add exchanges used by this bot
            if hasattr(bot, 'config') and isinstance(bot.config, dict):
                exchange_configs = bot.config.get('exchanges', [])
                for exchange in exchange_configs:
                    if isinstance(exchange, dict):
                        exchange_id = exchange.get('exchange_id')
                        if exchange_id:
                            accounts_dict[account_name]["exchanges"].add(exchange_id)
        
        # If no bots found, create a default account with all connected exchanges
        if not accounts_dict:
            # Create default "Main Account" with all available exchanges
            connected_exchanges = set()
            for conn in connections:
                if conn.has_credentials:
                    connected_exchanges.add(conn.connection_id)
            
            accounts_dict["Main Account"] = {
                "name": "Main Account",
                "exchanges": connected_exchanges,
                "symbols": {"BERA/USDT"},  # Default symbol
                "strategies": {"manual"},
                "bot_count": 0,
                "running_bots": 0
            }
        
        # Convert accounts dict to list format
        accounts = []
        for account_name, account_data in accounts_dict.items():
            # Get exchange details
            exchange_list = []
            for exchange_id in account_data["exchanges"]:
                # Find connection info
                conn = next((c for c in connections if c.connection_id == exchange_id), None)
                if conn and conn.has_credentials:
            ...
            # If no exchanges from bots, add all connected exchanges
            ...
            if account_data["running_bots"] > 0:
                status = "active"
            elif account_data["bot_count"] > 0:
                status = "inactive"
            elif any(ex["connected"] for ex in exchange_list):
                status = "ready"
            else:
                status = "error"
            ...
            account = {
                ...
                "status": status,
                "last_sync": None,  # Could be populated from trade sync logs
                "description": f"Trading account with {len(account_data['strategies'])} strategies",
                "active_symbols": list(account_data["symbols"]) if account_data["symbols"] else ["BERA/USDT"],
                "strategies": list(account_data["strategies"]),
                "bot_count": account_data["bot_count"],
                "running_bots": account_data["running_bots"]
            }
...
async def sync_trades(
    ...
    try:
        ...
        # Get account information to validate it exists
        accounts_response = await get_accounts(bot_manager, exchange_manager)
        account_names = [acc["name"] for acc in accounts_response["accounts"]]
        
        if account not in account_names:
            raise HTTPException(
                status_code=404,
                detail=f"Account {account} not found. Available accounts: {', '.join(account_names)}"
            )
        
        # Get the specific account details
        account_info = next((acc for acc in accounts_response["accounts"] if acc["name"] == account), None)
        ...
        # Get connected exchanges for the specific account
        all_connections = exchange_manager.get_all_connections()
        
        # Filter connections based on the account's exchanges
        account_exchange_ids = [ex["connection_id"] for ex in account_info["exchanges"]]
        active_connections = [
            conn for conn in all_connections 
            if conn.connection_id in account_exchange_ids
            and conn.has_credentials 
            and conn.connector 
            and conn.status.value == "connected"
        ]
        ...
        for exchange_id in all_exchanges:
            try:
                ...
                for result in pagination_results:
                    ...
                    # Symbol normalization is now handled within enhanced_trade_sync
