...
from api.services.bot_manager import BotManager, BotStatus
...
class UpdateBotConfigRequest(BaseModel):
    """Request model for updating a bot's configuration."""
    config: Dict[str, Any]
...
async def get_bots(bot_manager: BotManager = Depends(get_bot_manager)):
    ...
    try:
        ...
        # Convert instances to dict with live performance data
        instances_data = []
        for instance in instances:
            instance_dict = await instance.to_dict(bot_manager.redis_client)
...
async def get_bot(instance_id: str, bot_manager: BotManager = Depends(get_bot_manager)):
    ...
    try:
        ...
        return await instance.to_dict(bot_manager.redis_client)
...
async def create_bot(
    ...
    try:
        ...
        instance = bot_manager.get_instance(instance_id)
        instance_dict = await instance.to_dict(bot_manager.redis_client) if instance else None
        return {
...
async def get_bot_status_summary(bot_manager: BotManager = Depends(get_bot_manager)):
    ...
    try:
        ...
        return {
            ...
            "running_details": [await instance.to_dict(bot_manager.redis_client) for instance in running_instances]
        }
    except Exception as e:
...
async def get_bot_detailed_status(
    ...
    status = await instance.to_dict(bot_manager.redis_client)
...
async def get_bot_performance(
    ...
    perf_data = {
        ...
        "uptime_seconds": (
            (datetime.now(timezone.utc) - instance.started_at).total_seconds() 
...
@router.put("/{instance_id}/config", summary="Update bot configuration")
async def update_bot_config(
    ...
    If the bot is running, it will be stopped, updated, and restarted.
    """
    instance = bot_manager.get_instance(instance_id)
    if not instance:
        raise HTTPException(status_code=404, detail=f"Bot instance {instance_id} not found")

    was_running = instance.status == BotStatus.RUNNING

    try:
        # Stop the instance if it's running
        if was_running:
            logger.info(f"Stopping bot {instance_id} for configuration update...")
            await bot_manager.stop_instance(instance_id)

        # Update the configuration
        logger.info(f"Updating configuration for {instance_id}...")
        success = await bot_manager.update_instance_config(instance_id, request.config, request.exchanges)
        if not success:
            raise HTTPException(status_code=500, detail="Failed to update bot configuration file.")

        # Restart the instance if it was running before
        if was_running:
            logger.info(f"Restarting bot {instance_id} with new configuration...")
            await bot_manager.start_instance(instance_id)

        updated_instance = bot_manager.get_instance(instance_id)
        instance_dict = await updated_instance.to_dict(bot_manager.redis_client) if updated_instance else None
        return {
            "message": f"Bot instance {instance_id} configuration updated successfully.",
            "restarted": was_running,
            "instance": instance_dict
        }

    except Exception as e:
        logger.error(f"Error updating bot config for {instance_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e)) 
