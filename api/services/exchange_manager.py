...
class ExchangeManager:
    ...
    async def _test_connection(self, connection: ExchangeConnection) -> bool:
        ...
        try:
            ...
            # CRITICAL FIX: Bybit requires specific market_type values
            market_type = connection.exchange_type
            if connection.exchange_type == 'perp':
                if connection.name.lower() == 'bybit':
                    # Bybit uses 'linear' for USDT perpetuals
                    market_type = 'linear'
                else:
                    # Other exchanges use 'future' for perpetuals
                    market_type = 'future'
            ...
            try:
                ...
                # Propagate the full connection-id (e.g. binance_spot) so downstream
                # WebSocket / DB layers store the variant name, without mutating the
                # connector factory's kwargs (avoids duplicating the `testnet` arg).
                if connector and hasattr(connector, 'name'):
                    connector.name = connection.connection_id
    ...
    async def _test_all_connections(self) -> None:
        ...
        tasks = []
        for connection in self.connections.values():
            # For already connected exchanges, just do a health check
            if connection.status == ConnectionStatus.CONNECTED and connection.connector:
                task = asyncio.create_task(self._health_check_connection(connection))
            else:
                # For disconnected exchanges, do a full connection test
    ...
    async def _health_check_connection(self, connection: ExchangeConnection) -> bool:
        """
        Health check for an already connected exchange.
        This doesn't recreate the connection, just verifies it's still working.
        """
        try:
            # Quick health check - try to fetch balance
            if connection.connector:
            ...
            # Mark as disconnected and clear the connector
            connection.status = ConnectionStatus.DISCONNECTED
            connection.error_message = f"Health check failed: {str(e)}"
            connection.last_check = datetime.now(timezone.utc)
