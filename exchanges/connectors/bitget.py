...
Spot trading exchange with good API support and competitive fees.
"""
...
class BitgetConnector(BaseExchangeConnector):
    ...
    - Spot trading only
    - Good API stability
    - Copy trading features (not implemented here)
    ...
    def _init_exchange(self):
        ...
        try:
            self.exchange = ccxt.bitget({
                'apiKey': self.api_key,
                'secret': self.secret,
                'password': self.passphrase,  # Bitget requires passphrase
            ...
            self.logger.info("Initialized Bitget spot exchange")
            
        except Exception as e:
            self.logger.error(f"Failed to initialize Bitget exchange: {e}")
            raise
    
    async def connect(self) -> bool:
        """Connect to Bitget and load markets."""
        try:
            # Load markets
            ...
            self.connected = True
            self.logger.info(f"Connected to Bitget spot (server_time: {server_time})")
    ...
    async def disconnect(self) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Error disconnecting from Bitget: {e}")
            return False
    ...
    async def get_orderbook(self, symbol: str, limit: int = 100) -> Dict[str, Any]:
        ...
        try:
            ...
            orderbook = await self.exchange.fetch_order_book(bitget_symbol, limit)
    ...
    async def place_order(
        ...
        try:
            ...
            # Map clientOrderId to Bitget's clientOid field
            if order_params.get('clientOrderId'):
                order_params['clientOid'] = order_params.pop('clientOrderId')
            ...
            try:
                ...
                if order_type == "market":
                    order = await self.exchange.create_market_order(
                        bitget_symbol, side, amount_float, price_float, params=order_params
                    )
    ...
    async def cancel_order(self, order_id: str, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            order = await self.exchange.cancel_order(order_id, bitget_symbol)
            return self._normalize_order(order)
    ...
    async def get_order_status(self, order_id: str, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            order = await self.exchange.fetch_order(order_id, bitget_symbol)
            return self._normalize_order(order)
    ...
    async def get_open_orders(self, symbol: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            bitget_symbol = self.normalize_symbol(symbol) if symbol else None
            
            orders = await self.exchange.fetch_open_orders(bitget_symbol)
    ...
    async def get_trade_history(
        ...
        try:
            ...
            # Limit the request to avoid large responses
            actual_limit = min(limit, 100)  # Bitget max is 100
            
            # For Bitget, try without any time parameters first to avoid conflicts
            if bitget_symbol:
                # Try without since parameter to avoid time conflicts
                try:
                    trades = await self.exchange.fetch_my_trades(
                        bitget_symbol, None, actual_limit
                    )
                except Exception as time_error:
                    # If that fails and we have a since parameter, try a different approach
                    if since and "time" in str(time_error).lower():
                        # Use very recent time to avoid conflicts
                        recent_time = int((datetime.now(timezone.utc) - timedelta(hours=1)).timestamp() * 1000)
                        trades = await self.exchange.fetch_my_trades(
                            bitget_symbol, recent_time, actual_limit
                        )
                    else:
                        raise time_error
            else:
                # For all symbols, use the generic method without time parameters
                trades = await self.exchange.fetch_my_trades(
                    None, None, actual_limit
                )
            
            # If we have a since filter and got trades, filter them manually
            if since and trades:
                since_timestamp = int(since.timestamp() * 1000)
                trades = [trade for trade in trades if trade.get('timestamp', 0) >= since_timestamp]
    ...
    async def get_positions(self, symbol: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        return []  # Bitget only supports spot trading in this implementation
    
    def normalize_symbol(self, symbol: str) -> str:
        """Convert standard symbol format to Bitget format."""
        # Bitget uses standard format: BTC/USDT
        return symbol
    ...
    def _normalize_trade(self, trade: Dict) -> Dict[str, Any]:
        ...
        logger.debug(f"[BITGET] Raw trade data: id={trade.get('id')} raw={trade}")
        with open('logs/bitget_connector_raw_trades.log', 'a') as f:
            f.write(f"[BITGET] Raw trade data: id={trade.get('id')} raw={trade}\n")
        
        # Extract client_order_id from info.clientOid for Bitget
        info = trade.get('info', {})
