...
class BybitWSConnector:
    ...
    def __init__(
        ...
        # Logger
        self.logger = logger.bind(exchange="bybit")
    ...
    def _create_exchange(self):
        ...
        try:
            self.exchange = ccxt.bybit({
                ...
                'rateLimit': int(120 * self.rate_limit_factor),  # 120ms default with factor
                'timeout': self.request_timeout,
                'options': {
                    'defaultType': 'spot',  # Default to spot trading
                }
            })
            self.logger.info("Bybit CCXT exchange initialized")
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Failed to initialize Bybit connector: {e}")
            raise
