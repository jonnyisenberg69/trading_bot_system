...
Provides WebSocket and REST API functionality for the Hyperliquid exchange.
"""
...
class HyperliquidWSConnector:
    """
    Hyperliquid WebSocket connector for market data.
    
    Provides:
    - Real CCXT Hyperliquid API integration
    ...
    def __init__(
        ...
        # Logger
        self.logger = logger.bind(exchange="hyperliquid")
        
        # Initialize real CCXT exchange
        self._create_exchange()
        
    def _create_exchange(self):
        """Create real CCXT Hyperliquid exchange instance."""
        try:
            self.exchange = ccxt.hyperliquid({
                ...
                'rateLimit': int(200 * self.rate_limit_factor),  # 200ms default with factor
                'timeout': self.request_timeout,
                'options': {
                    'defaultType': 'spot',  # Hyperliquid supports both spot and perp
                }
            })
            self.logger.info("Hyperliquid CCXT exchange initialized")
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self.logger.info("Hyperliquid connector initialized successfully")
            
        except Exception as e:
            self.logger.error(f"Failed to initialize Hyperliquid connector: {e}")
            raise
    ...
    async def load_markets(self, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            # Load markets
            self.logger.info("Loading Hyperliquid markets...")
            ...
            self.logger.info(f"Loaded {len(markets)} Hyperliquid markets "
                           f"(spot: {spot_count}, swap: {swap_count})")
            return markets
            
        except Exception as e:
            self.logger.error(f"Failed to load Hyperliquid markets: {e}")
            raise
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            self.logger.info("Hyperliquid connector closed")
