...
class MexcConnector(BaseExchangeConnector):
    ...
    def _normalize_trade(self, trade: Dict) -> Dict[str, Any]:
        ...
        logger.debug(f"[MEXC] Raw trade data: id={trade.get('id')} raw={trade}")
        with open('logs/mexc_connector_raw_trades.log', 'a') as f:
            f.write(f"[MEXC] Raw trade data: id={trade.get('id')} raw={trade}\n")
        
        # Extract client_order_id from info.clientOrderId for MEXC
        info = trade.get('info', {})
