...
class GateIOConnector(BaseExchangeConnector):
    ...
    def _init_exchange(self):
        ...
        try:
            self.exchange = ccxt.gateio({
                ...
                'rateLimit': 50,  # 50ms between requests (more conservative)
    ...
    async def connect(self) -> bool:
        ...
        try:
            # First test public connectivity without loading all markets
            try:
                server_time = await self.exchange.fetch_time()
                self.logger.info(f"Gate.io server reachable, time: {server_time}")
            except Exception as time_error:
                self.logger.error(f"Cannot reach Gate.io server: {time_error}")
                self.connected = False
                return False
            
            # Then try to load markets with better error handling
            try:
                await self.exchange.load_markets()
                self.logger.info(f"Loaded {len(self.exchange.markets)} markets from Gate.io")
            except Exception as market_error:
                # Log the specific error but don't fail completely
                self.logger.error(f"Failed to load markets from Gate.io: {market_error}")
                # Try to continue anyway as we might still be able to trade
                # on specific pairs even if we can't load all markets
    ...
    def _normalize_trade(self, trade: Dict) -> Dict[str, Any]:
        ...
        logger.debug(f"[GATEIO] Raw trade data: id={trade.get('id')} raw={trade}")
        with open('logs/gateio_connector_raw_trades.log', 'a') as f:
            f.write(f"[GATEIO] Raw trade data: id={trade.get('id')} raw={trade}\n")
        
        # Extract client_order_id from info.text for GateIO
        info = trade.get('info', {})
        client_order_id = info.get('text', '')
