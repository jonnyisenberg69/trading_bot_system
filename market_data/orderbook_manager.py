...
from market_data.aggregator import OrderbookAggregator
...
class OrderbookManager:
    ...
    def __init__(self, websocket_manager: Optional[WebSocketManager] = None, aggregator: Optional[OrderbookAggregator] = None):
        """
        Initialize orderbook manager.
        
        Args:
            websocket_manager: WebSocket manager instance (created if None)
            aggregator: Orderbook aggregator instance (created if None)
        """
        ...
        self.aggregator = aggregator or OrderbookAggregator()
        self.aggregator.register_exchange_orderbooks(self.orderbooks, self._get_exchange_market_types())
    ...
    async def add_exchange(self, exchange: BaseExchangeConnector, symbols: List[str] = None):
        ...
        """
        # Use WebSocket manager's method to get proper exchange name
        exchange_name = self.ws_manager._get_exchange_name(exchange).lower()
    ...
    async def subscribe_symbol(self, exchange: str, symbol: str) -> bool:
        ...
        # Subscribe to WebSocket channel - use "orderbook" channel instead of "depth"
        conn_id = self.connections[exchange]
        result = await self.ws_manager.subscribe(conn_id, "orderbook", symbol)
    ...
    async def unsubscribe_symbol(self, exchange: str, symbol: str) -> bool:
        ...
        # Unsubscribe from WebSocket channel - use "orderbook" channel instead of "depth"
        conn_id = self.connections[exchange]
        result = await self.ws_manager.unsubscribe(conn_id, "orderbook", symbol)
    ...
    async def _handle_orderbook_message(self, message: Dict[str, Any]):
        ...
        self.logger.debug(f"Received orderbook message: exchange={exchange}, symbol={symbol}, type={message.get('type')}")
        
        if not exchange or not symbol:
            self.logger.warning(f"Invalid orderbook message: missing exchange or symbol - {message}")
            return
        
        # Skip if we're not tracking this orderbook
        if exchange not in self.orderbooks or symbol not in self.orderbooks[exchange]:
            self.logger.debug(f"Skipping orderbook message for {symbol} on {exchange} - not tracking")
            return
        ...
        if message_type != WSMessageType.ORDERBOOK:
            self.logger.warning(f"Expected ORDERBOOK message type, got {message_type}")
            return
        ...
        if is_snapshot:
            ...
            if not success:
                ...
                self.logger.debug(f"Applied orderbook delta for {symbol} on {exchange}")
        
        # After updating the orderbook:
        self.aggregator.update_combined_orderbooks(symbol)
        self.logger.debug(f"Updated combined orderbooks for {symbol}")
    ...
    def _get_exchange_market_types(self) -> Dict[str, str]:
        # Try to infer market type from exchange name
        result = {}
        for ex in self.orderbooks.keys():
            if 'perp' in ex or 'future' in ex or 'futures' in ex:
                result[ex] = 'futures'
            else:
                result[ex] = 'spot'
        return result
