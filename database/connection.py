...
async def init_db(db_url=None):
    ...
    if 'sqlite' in db_url:
        ...
        # PostgreSQL configuration - use default pool for asyncpg
        engine = create_async_engine(
            db_url,
            echo=False,
            future=True,
            # Don't specify poolclass for asyncpg - it uses AsyncAdaptedQueuePool by default
            pool_size=20,
            max_overflow=30,
            pool_pre_ping=True,
    ...
    try:
        ...
        # If PostgreSQL fails, fallback to SQLite for development
        if 'postgresql' in db_url:
            logger.warning("PostgreSQL connection failed, falling back to SQLite")
            await close_db()  # Close failed PostgreSQL connection
