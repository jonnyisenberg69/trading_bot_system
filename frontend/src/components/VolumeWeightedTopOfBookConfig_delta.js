    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-gray-400 text-sm mb-2">Target Inventory Source</label>
            <select
              value={config.target_inventory_source || 'manual'}
              onChange={(e) => handleConfigChange('target_inventory_source', e.target.value)}
              className="w-full bg-gray-700 text-white px-3 py-2 rounded border border-gray-600 focus:border-blue-500 focus:outline-none"
            >
              <option value="manual">Manual</option>
              <option value="delta_calc">Delta Calc</option>
            </select>
          </div>

          {/* Prevent Unprofitable Trades */}
          <div className="flex items-center gap-3 bg-gray-700/40 rounded px-3 py-2">
            <input
              type="checkbox"
              checked={!!config.prevent_unprofitable_trades}
              onChange={(e) => handleConfigChange('prevent_unprofitable_trades', e.target.checked)}
              className="h-4 w-4 text-blue-600"
            />
            <div>
              <label className="block text-gray-200 text-sm">Prevent Unprofitable Trades</label>
              <p className="text-gray-500 text-xs">Block buying above avg short price or selling below avg long price.</p>
            </div>
          </div>
      ...
      {config.target_inventory_source === 'delta_calc' && (
        <div className="space-y-4">
          <h3 className="text-lg font-semibold text-white">Delta Targeting</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="block text-gray-400 text-sm mb-2">Delta Service URL</label>
              <input
                type="text"
                value={config.delta_service_url || ''}
                onChange={(e) => handleConfigChange('delta_service_url', e.target.value)}
                className="w-full bg-gray-700 text-white px-3 py-2 rounded border border-gray-600 focus:border-blue-500 focus:outline-none"
                placeholder="http://127.0.0.1:8085"
              />
            </div>
            <div>
              <label className="block text-gray-400 text-sm mb-2">Refresh Seconds</label>
              <input
                type="number"
                value={config.delta_refresh_seconds ?? 5}
                onChange={(e) => handleConfigChange('delta_refresh_seconds', parseFloat(e.target.value) || 0)}
                className="w-full bg-gray-700 text-white px-3 py-2 rounded border border-gray-600 focus:border-blue-500 focus:outline-none"
                min="1"
                step="1"
              />
            ...
            <div>
              <label className="block text-gray-400 text-sm mb-2">Delta Change Threshold (%)</label>
              <input
                type="number"
                value={config.delta_change_threshold_pct ?? 0.025}
                onChange={(e) => handleConfigChange('delta_change_threshold_pct', parseFloat(e.target.value) || 0)}
                className="w-full bg-gray-700 text-white px-3 py-2 rounded border border-gray-600 focus:border-blue-500 focus:outline-none"
                min="0"
                step="0.001"
              />
            ...
            <div>
              <label className="block text-gray-400 text-sm mb-2">Trade Fraction</label>
              <input
                type="number"
                value={config.delta_trade_fraction ?? 0.1}
                onChange={(e) => handleConfigChange('delta_trade_fraction', parseFloat(e.target.value) || 0)}
                className="w-full bg-gray-700 text-white px-3 py-2 rounded border border-gray-600 focus:border-blue-500 focus:outline-none"
                min="0"
                max="1"
                step="0.01"
            ...
            <div>
              <label className="block text-gray-400 text-sm mb-2">Target Sign</label>
              <select
                value={String(config.delta_target_sign ?? -1)}
                onChange={(e) => handleConfigChange('delta_target_sign', parseFloat(e.target.value))}
                className="w-full bg-gray-700 text-white px-3 py-2 rounded border border-gray-600 focus:border-blue-500 focus:outline-none"
              >
                <option value="-1">Short (negative)</option>
                <option value="1">Long (positive)</option>
              </select>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gray-700 rounded p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
          ...
          <div>
            ...
            <span className="text-white ml-2">{config.target_inventory_source || 'manual'}</span>
          </div>
          <div>
            <span className="text-gray-400">Delta Threshold:</span>
            <span className="text-white ml-2">{config.delta_change_threshold_pct ?? 0.025}</span>
          </div>
          <div>
            <span className="text-gray-400">Trade Fraction:</span>
            <span className="text-white ml-2">{config.delta_trade_fraction ?? 0.1}</span>
          </div>
          <div>
            <span className="text-gray-400">Prevent Unprofitable:</span>
            <span className="text-white ml-2">{config.prevent_unprofitable_trades ? 'Enabled' : 'Disabled'}</span>
          </div>
          <div>
