...
const createAPIFunction = (requestFunction) => {
  return async (...args) => {
    try {
      ...
      const wrapped = new Error(error.userMessage || 'An error occurred while communicating with the server.');
...
export const api = {
  ...
  bots: {
    ...
    updateConfig: createAPIFunction((id, config, exchanges) => 
      apiClient.put(`/api/bots/${id}/config`, { config, exchanges })
    ),
  },
  ...
  strategies: {
    ...
    // Aggressive TWAP specific
    aggressiveTwap: {
      getConfig: createAPIFunction(() => apiClient.get('/api/strategies/aggressive-twap/config')),
      updateConfig: createAPIFunction((config) => apiClient.post('/api/strategies/aggressive-twap/config', config)),
      getPresets: createAPIFunction(() => apiClient.get('/api/strategies/aggressive-twap/presets')),
      savePreset: createAPIFunction((name, config) => apiClient.post(`/api/strategies/aggressive-twap/presets/${name}`, config)),
      deletePreset: createAPIFunction((name) => apiClient.delete(`/api/strategies/aggressive-twap/presets/${name}`)),
      createBot: createAPIFunction((request) => apiClient.post('/api/strategies/aggressive-twap/create', request)),
      quickUpdate: createAPIFunction((request) => apiClient.post('/api/strategies/aggressive-twap/quick-update', request)),
      getBotStatus: createAPIFunction((instanceId) => apiClient.get(`/api/strategies/aggressive-twap/${instanceId}/status`)),
    },
    validateSymbol: createAPIFunction((symbol, exchanges = null) => 
      apiClient.post('/api/strategies/validate-symbol', { symbol, exchanges })),
    getCurrentPrice: createAPIFunction((symbol, exchanges) => 
      apiClient.post('/api/strategies/get-current-price', { symbol, exchanges })),
    getAvailableStrategies: createAPIFunction(() => apiClient.get('/api/strategies/available-strategies')),
  },

  // Market making specific
  marketMaking: {
    // For now we use generic bot creation path with strategy field, but expose helpers
    createBot: createAPIFunction((request) =>
      apiClient.post('/api/bots/', {
        strategy: 'market_making',
        symbol: request.symbol,
        exchanges: request.config.exchanges || request.exchanges,
        config: request.config,
      })
    ),
    getBotStatus: createAPIFunction((instanceId) => apiClient.get(`/api/bots/${instanceId}`)),
  },
